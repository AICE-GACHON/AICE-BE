...
- [ ] 데모 한 번 미리 돌려보기 (첫 요청은 SPECTER2 로드로 수십 초)
- [ ] SPECTER2 모델이 이미 캐시됐는지 (`~/.cache/huggingface`) — 시연장 인터넷 없이 돌리려면 최소 한 번은 온라인에서 실행해 캐시해둘 것
- [ ] "AI 요약·태깅" 체크박스는 크레딧 소모 → `.env`에 `ANTHROPIC_API_KEY` 있을 때만 작동. 없으면 무료 모드로 시연

## 왜 원격 DB가 아니라 복사인가

- 방식 B는 어차피 노트북에 torch + SPECTER2 + 코드가 다 필요 → DB만 원격 둘 이점이 거의 없음
