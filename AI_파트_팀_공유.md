...
| `summary_markdown` | LLM이 쓴 종합 요약 (마크다운) | 그대로 렌더링 |

데모 화면(`demo/`)이 이 계약을 그대로 쓰는 참고 구현이다 — `demo/server.py`, `demo/static/`를 보면 각 필드를 어떻게 화면에 배치했는지 바로 알 수 있다. 실제 프론트가 준비되면 `demo/` 폴더는 통째로 지워도 된다(독립 폴더).
