...
| `docs/` | 공통 | 개발 문서 + 설계 근거 + 개편 계획 (아래 "문서" 참고) |
...
## ⚠️ 파이프라인 전면 개편이 계획돼 있습니다

아래 문서 전체는 **현재 코드 기준**입니다. 유사 논문 추천 파이프라인을 다음과 같이
바꾸는 계획이 확정돼 있고, **아직 착수하지 않았습니다**:

- 입력을 **PDF 전용**으로 (텍스트 붙여넣기 경로 삭제, `POST /api/submissions` 제거)
- 검색으로 후보 50편을 넓게 뽑고, **입력 PDF 원본과 후보 목록을 LLM에 함께 넘겨
  가장 비슷한 5편을 고르게 함** — 임베딩은 제목·초록만 보지만 LLM은 본문·참고문헌까지 본다
- **통계 레이어(`review_patterns`·`venue_trends`·`rating_context`) 제거** — 이 서비스의
  가치는 통계가 아니라 5편의 리뷰 원문이라는 판단

계획과 그 근거(실측 수치 포함)는 **[docs/추천_파이프라인_재설계.md](docs/추천_파이프라인_재설계.md)**
에 있습니다. 착수 전에 정해야 할 항목이 §12에 남아 있습니다.
...
- **[docs/추천_파이프라인_재설계.md](docs/추천_파이프라인_재설계.md)** — PDF 입력 + LLM
  재정렬로 가는 **계획**과 그 근거. 확정된 결정, 후보 수 N의 재현율 실측, PDF 토큰 비용
  실측, 착수 순서, 미확정 항목. **작업 시작 전에 읽을 것.**
...
- **검색이 aspect 예측에서 '검색 없음' 베이스라인에 집니다** (0.66~0.78배, §24).
  ...
  파이프라인 개편의 출발점입니다.
- **현재 파이프라인은 유사 논문을 제목+초록 임베딩만으로 고릅니다.** 본문·참고문헌을
  보지 않으므로, 같은 데이터셋을 쓰지만 접근이 전혀 다른 논문이 상위에 올 수 있습니다.
  개편안의 LLM 재정렬이 겨냥하는 지점입니다.
- **최신성 가중치는 순위를 크게 흔들지만 유사도는 거의 희생하지 않습니다.** 순수 유사도
  상위 5편 중 현행 top-30에 남는 건 70%뿐이지만, 실제로 보여주는 30편의 평균 코사인은
  **0.004밖에 낮지 않고**(0.9409 → 0.9370) 평균 1.2년 최신입니다. 이 차이는 SPECTER2가
  순위를 못 매기는 폭(상위 20편 내 0.013)보다도 작습니다 — 즉 **밀려난 논문과 들어온 논문을
  우리 임베딩은 구분하지 못합니다**(재설계 문서 §4.3). 그 구간을 실제로 판정하는 것이
  개편안의 LLM 재정렬입니다.
