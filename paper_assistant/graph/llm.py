...
예산이 $4.92뿐이라, LLM 호출은 명시적으로 켤 때만 실행된다.
`get_llm(enabled=False)`(기본)는 None을 반환하고, 노드는 이때 결정론적
스텁 출력을 생성한다 — DAG 배선을 $0으로 테스트하기 위함.
"""
import json
import logging
import os
...
def get_llm(enabled: bool | None = None) -> ClaudeLLM | None:
    ...
    enabled=None이면 환경변수 PAPER_ASSISTANT_USE_LLM=1 로 결정 (기본 off).
    off면 노드가 스텁 출력을 만든다 → 크레딧 소비 없음.
    """
    if enabled is None:
        enabled = os.getenv("PAPER_ASSISTANT_USE_LLM", "0") == "1"
    if not enabled:
        return None
    return ClaudeLLM()
