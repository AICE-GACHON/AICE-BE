"""논문 상세·리뷰 조회 — 유사 논문을 펼쳤을 때 보여줄 원문.

analyze()와 분리된 조회 API다. Report에 합치지 않는 이유는 schemas.PaperDetail
주석 참고 — 이웃 20편의 리뷰 전문은 대부분 열람되지 않는다.
...
def _review_rows_to_models(rows) -> list[ReviewDetail]:
    return [
        ReviewDetail(
...
def get_paper_detail(paper_id: int) -> PaperDetail | None:
    ...
    with cursor() as cur:
        ...
        reviews = _review_rows_to_models(cur.fetchall())
...
def get_paper_reviews(paper_id: int) -> list[ReviewDetail] | None:
    """리뷰만 조회한다. 논문이 없으면 None, 리뷰가 없으면 빈 리스트.

    get_paper_detail()로도 같은 리뷰를 얻을 수 있지만 그쪽은 저자·지적항목까지
    함께 읽는다. 리뷰만 필요한 화면에서 쓰지 않을 데이터를 조회하지 않도록
    쿼리 2개(존재 확인 + 리뷰)로 끝낸다.
    """
    with cursor() as cur:
        ...
        return _review_rows_to_models(cur.fetchall())
