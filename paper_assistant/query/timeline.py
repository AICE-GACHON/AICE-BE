...
**리뷰 시각은 DB에 없다.** reviews 테이블에 cdate 컬럼이 없고 raw_content도 비어
있어(실측: NOT NULL인 행 0건), 시각을 얻으려면 OpenReview forum replies를 실시간
조회하는 수밖에 없다. 그래서 이 모듈은 네트워크를 탄다 — 리뷰 본문·점수는 DB에서
