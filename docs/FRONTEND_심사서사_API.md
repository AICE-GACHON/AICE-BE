...
⚠️ 아래 필드 설명은 이해를 돕기 위한 것이고, **타입·nullable 여부가 정확히 필요하면
Swagger(`paper_assistant/schemas.py`에서 자동 생성)를 확인하세요.** 여기 남기는 건
스키마가 안 알려주는 것 — 언제 부를지, 왜 세 부분이 독립적으로 실패하는지, 화면에
옮길 때 어떤 실수를 하기 쉬운지입니다.
