...
| 6 | (조사했으나 기능 자체를 안 만들기로 함) 라벨 없는 수식을 자동으로 크롭하는 기능 | 인라인 수식 조각이 단독 수식으로 오탐되는 경우가 많고, 안전한 일반 규칙을 못 찾음 | ❌ 전체 되돌림 — 라벨 있는 수식만 크롭, 나머지는 텍스트로 유지 |
| 7 | (조사했으나 기능 자체를 안 만들기로 함) 같은 라벨 그림/표의 내용이 실제로 바뀌었는지 유사도로 자동 판정 | 설계 의도 자체가 "전/후 나란히 보여주고 사람이 판단"이라 시스템 판정은 범위 밖 | ❌ 되돌림 — `FieldChange.similarity`는 image kind에 안 씀 |
...
규칙을 못 찾겠으면 기능을 추가하기보다 되돌리는 쪽을 택한다(6절의 6·7번
항목이 그 예).
