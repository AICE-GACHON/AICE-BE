...
| 6 | Table 4의 크롭 이미지가 자기 헤더는 빠진 채, 데이터 아래에 **다음 표(Table 5)의 헤더 행까지 통째로 중복**해서 붙어 나옴. 23091에서도 Table 6 크롭에 Table 5 데이터가 그대로 중복 | 캡션 하나가 서로 다른 두 표(grid 조각)에 이중으로 배정될 수 있었음 — "캡션에 더 가까운 쪽"만 보고 고르다 보니, 이미 앞 표가 정당하게 쓰고 있는 캡션을 뒤 표(또는 뒤 표의 grid 조각)가 또 골라버리고, 같은 라벨은 하나로 합치는 로직(`merged[label]` 병합)을 타면서 두 표 영역이 하나로 뭉쳐짐. 괘선 기반 판단(algorithm 없는 표)에서도 같은 패턴 재현 | ✅ 고침 — 캡션(`grid_captions`)과 괘선(`raw_rules`) 둘 다 **한 번 배정되면 소모**되도록 추적(`used_caption_ids`, `used_rule_segments`). 단, 괘선 쪽은 좌우로 나란히 배치된 표/알고리즘까지 오소거되는 회귀가 있어 캡션의 가로 범위와 겹치는 괘선만 "이 캡션이 썼다"고 표시하도록 보정. 헤더 행이 여러 줄 겹쳐 빠지는 경우까지 잡도록 상단 확장도 반복형으로 변경 |
...
6번 수정은 "캡션·괘선은 표 하나에만 쓴다"는 가정에 기대는데, 이 가정이 깨지는
레이아웃(예: 캡션 하나가 정말로 두 표에 공유되는 극히 드문 편집 스타일)이
있다면 다시 깨질 수 있다 — 지금까지 실측한 3개 논문·모든 페이지에서는 이
가정이 항상 성립했다(회귀 검증: 겹치는 영역 0건, 좌우 배치 알고리즘도 정상).

---

## 7. 프론트엔드 통합 — 테스트 페이지 vs 실 서비스 화면

지금은 `AICE-FE/src/dev/BodyDiffTest.jsx`라는 **개발자용 테스트 페이지**에만
붙어 있다. 실 서비스 화면(`ResultReport.jsx` → `PaperStoryPanel`)에 넣을지는
아직 결정 사항이 아니라 아래 트레이드오프를 알고 판단하자는 것.

**body-diff 기능 자체는 LLM을 안 써서 몇 번을 열어도 0원이다.** 문제는 통합
위치다 — `ResultReport.jsx`는 논문을 열면 **자동으로 `PaperStoryPanel`이
`/story`를 호출**하는데, 이건 Haiku 요약이 붙어 비용이 든다. 그래서:

- **지금처럼 `/dev` 테스트 페이지 유지** → body-diff만 단독으로 열림, LLM 비용
  0원. UI는 개발자용이라 투박함.
- **`ResultReport.jsx`에 통합** → body-diff 자체는 여전히 0원이지만, 같은
  화면에 있는 `/story` 호출이 같이 따라와 논문당 1회(캐시 30일) 비용이 붙는다.
  단, 이건 "새로 생기는" 비용이 아니라 원래 그 화면을 열면 어차피 나가던
  비용이라 body-diff 때문에 늘어나는 몫은 아니다.

**정한 방향**: 팀에 먼저 보여줄 때는 `/dev` 테스트 페이지로 충분하다 — 굳이
`ResultReport.jsx` 통합(레이아웃 설계, 기존 diff UI와 자리 배치 등 별도
작업량)을 먼저 끝낼 필요는 없다. 피드백을 받은 뒤에 실 서비스 화면 통합
작업을 이어간다.
