...
| 6 | **잠재된 리스크(예상 오류)** — 지금까지 검증한 3개 논문에서는 안 나타났지만, **아직 안 넣어본 다른 논문을 코퍼스에 넣었을 때** 위 수정들의 전제가 깨지면 재발할 수 있는 유형 | 위 수정들은 전부 "검증 후 적용"(5번) 또는 "한 번 쓰인 캡션·괘선은 재사용 안 함"(6번, git 로그 참고) 같은 안전장치가 있지만 완전히 일반적인 증명은 아니다 — (a) 진짜 문장이 짧은 대문자 약어 5개 이상으로 끝나는 극단적 레이아웃이면 `get_textbox()`에도 똑같이 나타나 트림이 안 걸릴 수 있다. (b) 캡션 하나가 정말로 두 표에 공유되는 극히 드문 편집 스타일이면 "캡션·괘선은 표 하나에만 쓴다"는 가정이 깨질 수 있다 | ⚠️ 새 논문 투입 시 지켜봐야 함 — 3개 논문·5개 리비전·모든 페이지 전체 스캔으로 오탐/회귀 0건은 확인했지만, 다른 논문에서 이 패턴이 나타날 가능성 자체를 0으로 만든 건 아니다 |
...
(표에 없는 상세 수정 이력 — 표/알고리즘 캡션이 이웃 표의 캡션·괘선을 훔쳐가는
버그, 28599 Table 4/23091 Table 6 크롭에 다른 표 내용이 중복되던 것 —은
git 커밋 이력(`fix(pdf): 표/알고리즘 캡션이 이웃 표의 캡션·괘선을 훔쳐가는
버그를 고친다`)에서 자세히 볼 수 있다.)
...
`selectedPaperId`를 세팅해 `PaperStoryPanel`을 띄운다. 지금은 `.story-overlay`
(화면을 덮는 반투명 배경) 위에 `.story-panel` 하나만 뜨는 구조라, 화면
오른쪽에 심사 서사(요약·리뷰어 요구사항·저자 대응)가 나타나고 왼쪽은
어두워진 배경만 보인다.

### 결정된 방향

**같은 "심사 과정 보기" 클릭 한 번**으로 오른쪽엔 지금 그대로 `PaperStoryPanel`이
뜨고, **왼쪽에 `/dev/BodyDiffTest.jsx`처럼 본문·그림·표·알고리즘 diff를 보여주는
패널을 추가로 띄운다** — 별도 버튼이나 두 번째 클릭 없이 한 화면에서 "이 논문이
왜 이런 리뷰를 받았나(오른쪽)"와 "실제로 뭐가 바뀌었나(왼쪽)"를 동시에 보게
한다.

구현 방향(파일 단위):
- `api/papers.js`의 `getPaperRevisionsBodyDiff`는 이미 있다 — 그대로 재사용.
- `PaperStoryPanel.jsx`(또는 그 부모인 `ResultReport.jsx`의 오버레이 렌더
  부분)를 왼쪽 본문 패널 + 오른쪽 기존 패널의 **2단 레이아웃**으로 바꾼다.
  지금처럼 단일 `.story-panel`이 아니라, 왼쪽 폭을 넓게 잡은 새 컴포넌트
  (가칭 `PaperBodyDiffPanel`)를 나란히 둔다.
- 왼쪽 패널의 내용물은 `BodyDiffTest.jsx`가 이미 하고 있는 렌더링 로직
  (리비전별 세그먼트 색칠, `(그림 N)`/`(표 N)` 자리표시자를 실제 이미지로
  치환해 보여주는 부분)을 그대로 가져와 재사용한다 — 처음부터 새로
  만들 필요는 없다.
- `App.jsx`의 `?dev=body-diff` 진입 분기와 `BodyDiffTest.jsx` 자체는, 위
  통합이 끝나고 검증되면 주석에 적힌 대로 지운다.

**LLM 비용은 이 통합으로 늘지 않는다.** body-diff 자체는 여전히 0원이고,
오른쪽 `PaperStoryPanel`의 `/story` 호출(Haiku, 논문당 1회·30일 캐시)은
"심사 과정 보기"를 누르면 지금도 어차피 나가던 호출이라 body-diff를 왼쪽에
추가한다고 새로 생기는 비용이 아니다.

---

## 8. 디버깅 방법론 — 다음에 이상한 diff를 발견하면
