...
> ⚠️ **이 문서는 동결(freeze)됐습니다 (2026-08-19).** §1~26이 남긴 기록은 계속
> 유효하지만, 여기에 더 이상 새 섹션을 append하지 않습니다 — 단일 파일이 계속
> 커지면서 실제로 섹션 번호가 충돌하는 사고가 났었습니다(§20/§21, 이제 수정됨).
> **새 설계 결정은 [docs/adr/](adr/)에 결정 하나당 파일 하나로 기록하세요.**
> 규칙은 [docs/adr/README.md](adr/README.md) 참고.
...
`clustering.py._greedy_cluster`는 이 시점엔 범용 유틸로 남겨뒀으나(향후 aspect 내부
세분화 등을 염두), 1년이 지나도 쓰일 곳이 없어 **§22.1에서 삭제했다** — 지금은
존재하지 않는다.
...
### 21.2 대체 1 — 쿼리 단위 신뢰도 (진짜 신호가 여기 있다)
...
### 26.1 왜 임베딩만으로는 못 고르는가 — §21의 재확인

§21이 "논문별 유사도 점수를 만들 수 없다"고 한 근거(상위 20편 코사인 폭 0.013)는
...
(-0.0039), 최저는 -0.0008, 대신 평균 1.2년 최신이다. -0.004는 §21이 "순위를 정당화할
수 없다"고 판정한 폭보다도 작다.
