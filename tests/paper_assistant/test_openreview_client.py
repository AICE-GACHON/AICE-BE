...
재시도 정책의 경계는 파일 맨 아래에 함께 고정해 두었다.
"""
import json
...
def test_timeout_is_not_retried_and_propagates(cached_token):
    """재시도는 응답을 받은 경우에만 적용된다 — 타임아웃은 호출부로 올라간다.

    `_http.request_with_retry` 의 재시도 판단(`decide`)은 응답을 받은 뒤에 돈다.
    따라서 `session.request` 자체가 던지는 예외(타임아웃·연결 끊김)는 재시도
    대상이 아니라 그대로 전달된다. 재시도할지 말지는 호출부가 정한다.

    이 경계를 바꾸려면 `_http` 가 `requests.RequestException` 을 잡아야 하는데,
    그 함수는 arXiv·S2 클라이언트도 함께 쓰므로 세 수집 경로의 동작이 같이
    바뀐다. 그래서 정책 변경으로 다루고 별도로 논의한다.

    이 테스트가 깨진다면 그 정책이 바뀐 것이니 기대값을 함께 갱신할 것.
    """
