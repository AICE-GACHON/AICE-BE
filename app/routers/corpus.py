...
#
# 100/hour 근거: 분석 1회가 논문 5편을 내놓으므로 한 사람이 전부 열어봐도 5회다.
# 20번 분석해야 걸린다. 반면 paper_id를 1부터 훑는 스캔은 100회에서 멈춘다.
#
# ⚠️ **원래 30이었는데 100으로 올렸다. 그 값이 낮아서가 아니라, 지금까지 한 번도
# 걸린 적이 없었기 때문이다.** slowapi의 기본 key_style="url"이 URL마다 바구니를
# 따로 만들어서, paper_id가 다르면 서로 다른 30회였다(app/core/rate_limit.py).
# 2026-08-17에 그걸 "endpoint"로 고치면서 이 숫자가 **처음으로 진짜 상한이 됐다.**
# 30을 그대로 뒀다면 결과 6건만 열어봐도 정상 사용자가 429를 맞았을 것이다.
#
# ⚠️ 저장소가 메모리라 워커를 늘리면 워커별로 따로 센다 (app/core/rate_limit.py).
# 배포에서 워커를 늘릴 거면 Redis 저장소로 바꿔야 이 상한이 실제로 상한이 된다.
_EXTERNAL_CALL_LIMIT = "100/hour"
...
# `/revisions`(100회)의 3분의 1인 30회에서 끊는다.
#
# ⚠️ 이것도 10 → 30이다. 위와 같은 이유이고(지금까지 안 걸렸다), 비율은 그대로
# 유지했다 — 무거운 순서가 상한 순서에 남아 있어야 나중에 조정할 때 근거가 산다.
_BODY_DIFF_CALL_LIMIT = "30/hour"
...
def get_revisions(request: Request, paper_id: int):
    ...
    ⚠️ IP 기준 시간당 100회로 제한됩니다 (외부 API를 타기 때문). 429가 오면
    잠시 뒤 다시 시도하세요.
...
def get_revisions_body_diff(
    ...
    ⚠️ **IP 기준 시간당 30회로 제한됩니다** (`/revisions`의 100회보다 엄격 — 캐시
...
def get_story(
    ...
    - **IP 기준 시간당 100회.** 캐시된 논문을 다시 읽는 것은 비용이 없지만, 캐시에 없는
