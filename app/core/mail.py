...
--
이 메일은 발신 전용입니다. 회신하셔도 답변을 받지 못합니다.
"""

# 회신 불가 안내를 본문에 넣는 이유: noreply@ 도메인에 **수신용 MX 레코드가 없다.**
# 답장은 어디에도 도착하지 않고 발신자에게 반송된다. 안내가 없으면 사용자는
# 답장을 보내고 답을 기다린다.
#
# ⚠️ 문의처를 넣으려면 위 문구에 한 줄 더하면 된다. 지금은 **실제로 사람이 읽는
#    주소가 정해지지 않아** 비워 두었다 — 닿지 않는 주소를 적는 것은 안 적는
#    것보다 나쁘다. 주소가 정해지면 Reply-To 헤더도 함께 고려할 것.
